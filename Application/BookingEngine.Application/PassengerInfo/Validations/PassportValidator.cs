    ...
    public class PassportValidator : AbstractValidator<IValidatableDto>
    {
        public PassportValidator()
        {
            RuleSet("create", () =>
            {
                ...
                {
                    RuleFor(x => ((PassportCreateDto)x).DocID)
                        .NotEmpty().WithMessage("Document ID is required.")
                        .MaximumLength(20);

                    RuleFor(x => ((PassportCreateDto)x).ExpireDate)
                        .GreaterThan(DateTime.Today).WithMessage("Passport expiry date must be in the future.");
                });
            });
